...
function makeHandleResponse(logger, res, convertToBinary) {
    return function (err, response) {
        ...
        // Check whether response body is too big for AWS Lambda to return
        // Lambda has a response limit of 6MB
        if (Buffer.byteLength(response.body) > 6000000) {
            return res
                .status(500)
                .send('body size is too long');
        }
